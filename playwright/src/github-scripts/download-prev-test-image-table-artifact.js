/**
 * borrowed from: https://www.eliostruyf.com/retrieving-artifact-previous-github-actions-workflow/
 */
module.exports = async ({
  ...
  const workflow = workflows.data.workflows.find(w => w.path.includes('test-image-table.yml'));

  if (!workflow) {
    core.info("No workflow found");
    return;
  }
  ...
  if (artifact) {
    ...
    require('child_process').execSync(`unzip -o ${process.env.ARTIFACT_FILENAME} -d playwright`);
